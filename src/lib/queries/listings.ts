...
export async function getListings(
  ...
  let query = supabase
    .from("listings")
    .select("*, profiles!inner(id, display_name, avatar_url, district)", { count: "exact" })
    .eq("status", "aktiv")
...
export async function getListingById(id: string) {
  ...
  const { data, error } = await supabase
    .from("listings")
    .select("*, profiles!inner(id, display_name, avatar_url, district, email, phone, show_email, show_phone, bio, created_at)")
    .eq("id", id)
    .single();
...
export async function getFeaturedListings(limit = 6): Promise<ListingWithProfile[]> {
  ...
  const { data, error } = await supabase
    .from("listings")
    .select("*, profiles!inner(id, display_name, avatar_url, district)")
    .eq("status", "aktiv")
    .order("created_at", { ascending: false })
    .limit(limit);
